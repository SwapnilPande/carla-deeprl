...
class MapImage(object):
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points, recompute=False):
        ...
        save_path = f'assets/maps/{carla_map.name}'
        if recompute:
            self.draw_road_map(
                    self.big_map_surface, self.big_lane_surface, self.big_stop_surface, self.big_waypoint_surfaces,
            ...
            self.big_lane_surface = pygame.image.load_extended(f'{save_path}/lane.png')
        ...
        if recompute:
            ...
            pygame.image.save_extended(self.big_lane_surface, f'{save_path}/lane.png')
    ...
    def draw_road_map(self, map_surface, lane_surface, stop_surface, waypoint_surfaces, carla_world, carla_map, world_to_pixel, world_to_pixel_width, threshold=math.radians(1)):
        # map_surface.fill(COLOR_ALUMINIUM_4)
        map_surface.fill(COLOR_BLACK)
        lane_surface.fill(COLOR_BLACK)
        precision = 0.05
        ...
        def draw_traffic_signs(surface, actor):
            ...
            left_vector = carla.Location(-forward_vector.y, forward_vector.x,
                                         forward_vector.z) * waypoint.lane_width / 2 * 1.2

            line = [(waypoint.transform.location - (forward_vector * 1.5) + (left_vector)),
                    (waypoint.transform.location - (forward_vector * 1.5) - (left_vector))]

            line = [world_to_pixel(p) for p in line]
            # pygame.draw.lines(surface, COLOR_WHITE, True, line_pixel, 1)
            # l = tl.trigger_volume.location
            # start = carla.Location(l.x + v[0] * 0.5 - v[1] * 8, l.y + v[1] * 0.5 + v[0] * 8, l.z)
            # end   = carla.Location(l.x - v[0] * 4.5 - v[1] * 8, l.y - v[1] * 4.5 + v[0] * 8, l.z)
            # line = [world_to_pixel(p) for p in [start, end]]
            angle = waypoint.transform.rotation.yaw
            color = encode_orientation(angle)

            # pygame.draw.lines(surface, color, False, line, 10)

            # draw bounding box
            # if self.show_triggers:
            #     corners = Util.get_bounding_box(actor)
            #     corners = [world_to_pixel(p) for p in corners]
            #     pygame.draw.lines(surface, COLOR_WHITE, True, corners, 1)
        ...
        def draw_topology(carla_topology, index):
            ...
            for waypoint in topology:
                ...
                if len(polygon) > 2:
                    pygame.draw.polygon(lane_surface, COLOR_WHITE, polygon, 5)
                    pygame.draw.polygon(lane_surface, COLOR_WHITE, polygon)
            ...
            for wp in carla_map.generate_waypoints(dist):
                ...
                for nxt, deviation, next_cross in zip(next_wps, deviations, next_crosses):
                    ...
                    if np.min(np.linalg.norm(intersections-[wx, wy], axis=1)) > 0:
                        ...
                        #     angle = cur.transform.rotation.yaw
                        #     col = encode_orientation(angle)
...
class ModuleWorld(object):
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel, world_to_pixel_width):
        ...
        for tl in list_tl:
            ...
            waypoint = self.town_map.get_waypoint(transform.location)
            angle = waypoint.transform.rotation.yaw

            color = encode_orientation(angle)
            # US light trigger box

            if tl.state != tls.Red and tl.state != tls.Yellow:
                continue # Unknown or off traffic light

            scale_y = {'Town10HD': 2}.get(self.town_map.name, 10)
            corners = Util.get_bounding_box(tl,offset_x=0.5,offset_y=1.5, scale_x=1.2,scale_y=scale_y)
            corners = [world_to_pixel(p) for p in corners]
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel, world_to_pixel_width):
        ...
        for w in list_w:
            ...
            and abs(w[0].get_location().z - self.hero_actor.get_location().z) < 1:
                ...
                pos_x, pos_y =  world_to_pixel(world_pos + carla.Location(x=self.hero_x*np.cos(angle + math.pi), y=self.hero_x*np.sin(angle + math.pi)))
                radius = world_to_pixel_width(bb.x + self.hero_x + 1)
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        ego_wpt = self.town_map.get_waypoint(self.hero_actor.get_location())
        ...
        for v in list_v:
            ...
            if wpt.is_junction:
                ignore = not fairly_close
            elif not fairly_close:
                ignore = True
            else:
                if wpt.road_id != ego_wpt.road_id \
                or wpt.lane_id != ego_wpt.lane_id:
                    passed = True
                    cur_wpt = ego_wpt
                    for _ in range(5):
                        for next_wpt in cur_wpt.next(2.0):
                            if next_wpt.lane_change:
                                continue
                            if next_wpt.road_id == wpt.road_id and next_wpt.lane_id == wpt.lane_id:
                                passed = False
                                break
                        if not passed:
                            break
                    ...
                    if passed:
                        for next_wpt in ego_wpt.next(2.0):
                            if next_wpt.road_id == wpt.road_id and next_wpt.lane_id == wpt.lane_id:
                                passed = False
                                break
            ...
            if (ignore and not very_close) or not fairly_close:
                continue

            if abs(v[0].get_location().z - self.hero_actor.get_location().z) > 1:
                continue
            ...
            if hasattr(v[0], 'bounding_box'):
                ...
                corners = [carla.Location(x=-bb.x-self.hero_x-4, y=-bb.y-self.hero_y/2-1.2),
                          carla.Location(x=-bb.x-self.hero_x-4, y=bb.y+self.hero_y/2+1.2),
                          carla.Location(x=bb.x+0.8, y=bb.y+self.hero_y/2+1.2),
                          carla.Location(x=bb.x+0.8, y=-bb.y-self.hero_y/2-1.2)
                          ]
            else:
                corners = [carla.Location(x=-3.5, y=-6),
                          carla.Location(x=-3.5, y=6),
                          carla.Location(x=3.5, y=6),
                          carla.Location(x=3.5, y=-6)
                          ]

            angle = v[0].get_transform().rotation.yaw
            color = encode_orientation(angle)
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            self.hero_lane_surface.fill(COLOR_BLACK)
            self.hero_stop_surface.fill(COLOR_BLACK)
            ...
            self.hero_stop_surface.blit(self.map_image.stop_surface, (-offset[0], -offset[1]))
            ...
            self.hero_map_image = make_image(self.hero_map_surface)
            self.hero_lane_image = make_image(self.hero_lane_surface)
            self.hero_stop_image = make_image(self.hero_stop_surface)
            self.hero_waypoint_images = [make_image(surface) for surface in self.hero_waypoint_surfaces]
            self.hero_vehicle_image = make_image(self.hero_vehicle_surface)
            self.hero_walker_image = make_image(self.hero_walker_surface)
            self.hero_traffic_image = make_image(self.hero_traffic_light_surface)
        else:
            # Translation offset
...
class Wrapper(object):
    ...
    def get_observations(cls):
        road, lane, stop, vehicle, pedestrian, traffic, waypoints = cls.world_module.get_rendered_surfaces()
