...
class CollisionReader:
    def __init__(self, bp_library, vehicle, reading_frequency=1.0):
        self._collided = {"collision" : False, "actor" : None}
    ...
    def on_collision(weakself, data):
        self = weakself()
        self._collided = {"collision" : True, "actor" : data.other_actor}

### MODIFIED
class LaneInvasionReader:
    def __init__(self, bp_library, vehicle, reading_frequency=1.0):
        self._lane_invasion = False
        bp = bp_library.find('sensor.other.lane_invasion')
        self.sensor = CarlaDataProvider.get_world().spawn_actor(bp, carla.Transform(), vehicle)
        self.sensor.listen(lambda event: self.__class__._on_invasion(weakref.ref(self), event))
    ...
    def run(self):
        ...
        while self._run_ps:
            if self._callback is not None:
                ...
                if current_time - latest_time > (1 / self._reading_frequency) \
                        or (first_time and GameTime.get_frame() != 0):
                    self._callback(GenericMeasurement(self._lane_invasion, GameTime.get_frame()))
                    latest_time = GameTime.get_time()
                    first_time = False
                else:
                    time.sleep(0.001)
    ...
    @staticmethod
    def _on_invasion(weakself, data):
        self = weakself()
...
class CallBack(object):
    ...
    def __call__(self, data):
        if isinstance(data, carla.libcarla.Image):
            ...
            self._parse_lane_invasion_cb(data, self.tag)
    ...
    def _parser_collision_cb(self, event, tag):
        import ipdb; ipdb.set_trace()
        collision = event.intensity > 20
        other_actor = event.other_actor
        self._data_provider.update_sensor(tag, {"collision" : collision, "actor" : other_actor}, event.frame)


    def _parser_lane_invasion_cb(self, event, tag):
        ...
        if carla.libcarla.LaneMarkingType.NONE in lane_types:
            self._data_provider.update_sensor(tag, True, event.frame)
