...
class ReactiveDataset(Dataset):
    def __init__(self, path):
        ...
        for trajectory_path in self.trajectory_paths:
            ...
            action_value_paths = sorted(glob.glob("{}/action_value/*.npy".format(trajectory_path)))
    ...
    def __getitem__(self, idx):
        ...
        return wide_rgb, wide_seg, narrow_rgb, narrow_seg, rewards, values, action_values, world_pts
...
class ReactiveDatasetModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        datasets = [ReactiveDataset(path) for path in self.paths]
    ...
    def val_dataloader(self):
        return DataLoader(self.val_data, batch_size=32, shuffle=True, num_workers=4)
