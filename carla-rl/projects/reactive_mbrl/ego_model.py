...
class EgoModelRails(nn.Module):
    def __init__(self, dt=1.0 / 4):
        ...
        self.throt_accel = nn.Sequential(nn.Linear(1, 1, bias=False),)
    ...
    def forward(self, locs, yaws, spds, acts):
        ...
        accel = torch.where(
            brake, self.brake_accel.expand(*brake.size()), self.throt_accel(throt)
        )
...
class EgoModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for t in range(4):
            ...
            pred_loc, pred_yaw, pred_spd = self.forward(
                pred_loc, pred_yaw, pred_spd, act
            )
        ...
        ori_loss = F.l1_loss(torch.cos(pred_yaws), torch.cos(yaws[:, 1:])) + F.l1_loss(
            torch.sin(pred_yaws), torch.sin(yaws[:, 1:])
        )
