...
dataset['next_observations'] = np.load('data/next_observations.npy')
dataset['actions'] = np.load('data/actions.npy')
dataset['terminals'] = np.load('data/terminals.npy')
...
obs_chunks_train, action_chunks_train = chunks(states_train, next_states_train, actions_train, terminals_train, H, stride)

print('states_test.shape: ',states_test.shape)
print('MAKIN TEST SET!!!')

obs_chunks_test,  action_chunks_test  = chunks(states_test,  next_states_test,  actions_test, terminals_test, H, stride)


experiment = Experiment(api_key = 'wb7Q8i6WX2nuxXfHJdBSs9PUV', project_name = 'carla-skills')
