...
class ProbabilisticMLPCovDynamics(nn.Module):
    def __init__(self,
                state_dim_in,
                state_dim_out,
                frame_stack,
                ...
                drop_prob = 0,
                activation = nn.SiLU):
        super(ProbabilisticMLPCovDynamics, self).__init__()
    ...
    def forward(self, x, include_reward = False):
        ...
        for layer_hat in self.diag_head:
            diag_hat = layer_hat(diag_hat)
...
class ProbabilisticMLPCovDynamicsEnsemble(nn.Module):
    ...
    def __init__(self,
                    ...
                    logger = None,
                    log_freq = 100):
        super(ProbabilisticMLPCovDynamicsEnsemble, self).__init__()
        ...
        if logger is not None:
            self.logger.pickle_save(self.config, ProbabilisticMLPCovDynamicsEnsemble.log_dir, "config.pkl")
        ...
        for i in range(self.n_models):
            self.models.append(ProbabilisticMLPCovDynamics(
    ...
    def save(self, model_name):
        ...
        # Save model
        self.logger.torch_save(self.state_dict(), ProbabilisticMLPCovDynamicsEnsemble.model_log_dir, model_name)
    ...
    def load(cls, logger, model_name, gpu):
        ...
        config = logger.pickle_load(ProbabilisticMLPCovDynamicsEnsemble.log_dir, "config.pkl")

        # Next, get pickle containing the state parameters
        state_dim_in, state_dim_out, action_dim, frame_stack, norm_stats = logger.pickle_load(ProbabilisticMLPCovDynamicsEnsemble.log_dir, "state_params.pkl")
