...
def gen_state_pic(cur_state, next_state, ensemble_predictions, save_dir, idx):
    ...
    plt.plot(next_x_predictions, next_x_predictions, "x", color = "orange")

    # Center agent on the plot
    plt.xlim(x-5, x+5)
    plt.ylim(y-5, y+5)

    plt.save(os.path.join(save_dir, "{:04d}.png".format(idx)))



def n_step_eval(logger, real_env, fake_env, policy, num_episodes, n = 1, generate_videos = False):
    # List to store rollouts errors
    # Store each step separately
    n_step_rollout_squared_errors = [0.0] * n
    n_step_reward_squared_errors = [0.0] * n
    ...
    for i in range(num_episodes):
        ...
        fake_obs = fake_env.reset(obs = real_obs)
        ...
        while not real_done:
            ...
            past_real_obs = real_obs
            past_fake_obs = fake_obs
            real_obs, real_reward, real_done, real_info =  real_env.step(action)
            ...
            n_step_rollout_squared_errors[fake_rollout_steps] = (real_obs - fake_obs)**2
            ...
            # if(generate_videos):
            #     gen_state_pic(past_real_obs, real_obs, ensemble_predictions, image_save_dir, real_rollout_steps)
...
class DynamicsEvaluationConf:
    def __init__(self):
        ...
        self.experiment_key = "c2b22966acdf49faa5e2578cd925e653"
...
def main(args):
    ...
    fake_env_config.populate_config(\
        obs_config = "VehicleDynamicsNoCameraConfig", \
        action_config = "MergedSpeedTanhConfig",\
        reward_config="Simple2RewardConfig")
