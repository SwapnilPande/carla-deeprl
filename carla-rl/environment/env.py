...
from environment.carla_interfaces.carla_interface import (
    Carla910Interface,
    Carla910Interface_Leaderboard,
)
...
class CarlaEnv(gym.Env):
    def __init__(
        ...
        self.top_rgb_stacked_observation_queue = queue.Queue(
            maxsize=self.frame_stack_size
        )
        self.top_sem_stacked_observation_queue = queue.Queue(
            maxsize=self.frame_stack_size
        )
        self.front_rgb_stacked_observation_queue = queue.Queue(
            maxsize=self.frame_stack_size
        )
        self.front_sem_stacked_observation_queue = queue.Queue(
            maxsize=self.frame_stack_size
        )
        ...
        for _ in range(self.config.action_config.frame_skip):
            ...
            if self.config.action_config.use_pid_in_frame_skip:
                ...
                if self.config.verbose:
                    print(
                        "steer",
                        carla_obs["control_steer"],
                        "throttle",
                        carla_obs["control_throttle"],
                        "brake",
                        carla_obs["control_brake"],
                        "reverse",
                        carla_obs["control_reverse"],
                    )
                    print("steps", self.num_steps)

                # Store control for this step
                self.episode_measurements["control_steer"] = carla_obs["control_steer"]
                self.episode_measurements["control_throttle"] = carla_obs[
                    "control_throttle"
                ]
                self.episode_measurements["control_brake"] = carla_obs["control_brake"]
                self.episode_measurements["control_reverse"] = carla_obs[
                    "control_reverse"
                ]
                self.episode_measurements["control_hand_brake"] = carla_obs[
                    "control_hand_brake"
                ]
            ...
            self.episode_measurements["next_orientation"] = carla_obs[
                "next_orientation"
            ]
            self.episode_measurements["control_steer"] = carla_obs["control_steer"]
            ...
            self.episode_measurements["distance_to_goal_trajec"] = carla_obs[
                "distance_to_goal_trajec"
            ]
            self.episode_measurements["speed"] = util.get_speed_from_velocity(
                carla_obs["ego_vehicle_velocity"]
            )
            self.episode_measurements["target_speed"] = carla_obs["target_speed"]

            self.episode_measurements["num_collisions"] = carla_obs["collision_sensor"][
                "num_collisions"
            ]
            self.episode_measurements["collision_actor_id"] = carla_obs[
                "collision_sensor"
            ]["collision_actor_id"]
            self.episode_measurements["collision_actor_type"] = carla_obs[
                "collision_sensor"
            ]["collision_actor_type"]
            if not self.config.obs_config.disable_lane_invasion_sensor:
                self.episode_measurements["num_laneintersections"] = carla_obs[
                    "lane_invasion_sensor"
                ]["num_lane_intersections"]
                self.episode_measurements["out_of_road"] = int(
                    carla_obs["lane_invasion_sensor"]["out_of_road"]
                )

            self.episode_measurements["distance_to_goal"] = carla_obs["dist_to_goal"]
            if (
                self.episode_measurements["min_distance_to_goal"]
                >= carla_obs["dist_to_goal"]
            ):
                self.episode_measurements["min_distance_to_goal"] = carla_obs[
                    "dist_to_goal"
                ]

            # Additional state vectors storing the position of the vehicle
            self.episode_measurements["ego_vehicle_x"] = carla_obs[
                "ego_vehicle_location"
            ].location.x
            self.episode_measurements["ego_vehicle_y"] = carla_obs[
                "ego_vehicle_location"
            ].location.y
            self.episode_measurements["ego_vehicle_theta"] = carla_obs[
                "ego_vehicle_location"
            ].rotation.yaw
            ...
            obs_collision = (
                self.episode_measurements["num_collisions"]
                - self.prev_measurement["num_collisions"]
                > 0
            )

            if (
                obs_collision
                and self.episode_measurements["collision_actor_id"]
                != self.prev_measurement["collision_actor_id"]
            ):
            ...
            if self.config.verbose:
                print(
                    "Collisions Total: {}, Vehicle: {}, Static: {}".format(
                ...
                print(
                    "Traffic Light Violations: {}".format(self.traffic_light_violations)
                )
            ...
            self.obstacle_speed_array.append(
                self.episode_measurements["obstacle_speed"]
            )
            self.wp_orientation_array.append(
                self.episode_measurements["next_orientation"]
            )
            ...
            self.red_light_dist_array.append(
                self.episode_measurements["red_light_dist"]
            )
            self.dist_to_trajectory_array.append(
                self.episode_measurements["dist_to_trajectory"]
            )
            self.dist_to_target_array.append(
                self.episode_measurements["distance_to_goal_trajec"]
            )
            self.step_reward_array.append(self.episode_measurements["step_reward"])
            self.collision_reward_array.append(
                self.episode_measurements["collision_reward"]
            )
            self.dist_to_trajectory_reward_array.append(
                self.episode_measurements["dist_to_trajectory_reward"]
            )
    ...
    def _update_straight_dynamic_obs(self):
        car_spawn_point = Transform(
            Location(x=92.10997772216797, y=249.42999267578125, z=1.32),
            Rotation(yaw=-90.00029754638672),
        )
        location = self.vehicle_actor.get_location()
        distance_to_car = location.distance(car_spawn_point.location)
    ...
    def _update_env_obs(self, front_rgb_image=None):
        ...
        if not self.config.scenario_config.disable_traffic_light:
            ...
            if self.config.verbose:
                print(
                    self.episode_measurements["dist_to_light"],
    ...
    def _update_obs_detector(self):
        ...
        for target_vehicle in self.carla_interface.get_actor_list():
            ...
            if (
                target_vehicle.id == ego_vehicle_actor.id
                or "vehicle" not in target_vehicle.type_id
            ):
                continue

            # if the object is not in our lane it's not an obstacle
            target_vehicle_waypoint = map.get_waypoint(target_vehicle.get_location())
            d_bool, d_angle, distance = self.is_within_distance_ahead(
            ...
            if not d_bool:
                ...
                if not util.check_if_vehicle_in_same_lane(
                    ego_vehicle_actor, target_vehicle, self.next_waypoints, map
                ):
                    continue
                ...
                if distance < min_obs_distance:
                    ...
                    self.episode_measurements[
                        "obstacle_speed"
                    ] = util.get_speed_from_velocity(target_vehicle.get_velocity())
    ...
    def _update_traffic_light_states(self):
        ...
        ) = ego_vehicle.find_nearest_traffic_light(traffic_actors)
        ...
        if traffic_actor is not None:
            if traffic_actor.state == carla.TrafficLightState.Red:
                ...
                if self.episode_measurements["initial_dist_to_red_light"] == -1 or (
                    self.episode_measurements["nearest_traffic_actor_id"] != -1
                    and traffic_actor.id
                    != self.episode_measurements["nearest_traffic_actor_id"]
                ):
    ...
    def create_observations_scalar(self):
        ...
        if self.config.obs_config.input_type == "wp_constant":
            ...
            obs_output = np.concatenate(
                (
                    ...
                    np.random.normal(0.0, 1.0, self.config.obs_config.noise_dim),
                )
            )

        elif self.config.obs_config.input_type == "wp_obs_dist":
            obs_dist = (
                self.episode_measurements["obstacle_dist"]
                / self.config.obs_space.obstacle_dist_norm
            )
            obs_output = np.concatenate(
                (
                    np.array([self.episode_measurements["next_orientation"]]),
            ...
            distance_to_goal_trajec = (
                self.episode_measurements["distance_to_goal_trajec"] / 500
            )
            obs_output = np.concatenate(
                (
                    np.array([self.episode_measurements["next_orientation"]]),
            ...
            obs_speed = self.episode_measurements["speed"] / 10
            distance_to_goal_trajec = (
                self.episode_measurements["distance_to_goal_trajec"] / 100
            )
            obs_output = np.concatenate(
                (
                    np.array(self.episode_measurements["next_orientation"]),
            ...
            self.config.obs_config.input_type
            == "wp_obs_info_speed_steer_ldist_goal_light"
        ):
        ...
        elif self.config.obs_config.input_type in [
            "wp_vae_obs_info_speed_steer_ldist_goal_light",
            "wp_cnn_obs_info_speed_steer_ldist_goal_light",
            "wp_bev_rv_obs_info_speed_steer_ldist_goal_light",
        ]:
            speed = self.episode_measurements["speed"] / 10
            obstacle_dist = self.episode_measurements["obstacle_dist"]
            obstacle_speed = self.episode_measurements["obstacle_speed"]
    ...
    def _reset(self, unseen=False, index=0, expert_agent=False):
        ...
        self.episode_measurements["collision_actor_id"] = carla_obs["collision_sensor"][
            "collision_actor_id"
        ]
        self.episode_measurements["collision_actor_type"] = carla_obs[
            "collision_sensor"
    ...
    def get_autopilot_action(self, target_speed=0.5):
        hazard_detected = (
            self.carla_interface.actor_fleet.ego_vehicle.check_for_hazard()
        )
        if hazard_detected:
            return np.array([0, -1])
        else:
            waypoint = self.carla_interface.next_waypoints[0]
            steer = self.carla_interface.actor_fleet.lateral_controller.pid_control(
                waypoint
            )
    ...
    def get_wp_obs_input(self):
        ...
        if n == 0:
            ...
            print(
                "Error: More than {0} waypoints returned from planner.".format(num_wp)
            )
    ...
    def _compute_done_condition(self):
        ...
        success = (
            ...
            < self.config.scenario_config.dist_for_success
        )
        offlane = (
            self.episode_measurements["offlane_steps"]
            > self.config.scenario_config.max_offlane_steps
        )  # Unused

        # Check if static threshold reach, always False if static is disabled
        static = (
            self.episode_measurements["static_steps"]
            > self.config.scenario_config.max_static_steps
        ) and not self.config.scenario_config.disable_static

        # Check if collision, always False if collision is disabled
        collision = (
            self.episode_measurements["is_collision"]
            and not self.config.scenario_config.disable_collision
        )
        runover_light = (
            self.episode_measurements["runover_light"]
            and not self.config.scenario_config.disable_traffic_light
        )
        maxStepsTaken = (
            self.episode_measurements["num_steps"]
            > self.config.scenario_config.max_steps
        )
        offlane = (
            self.episode_measurements["num_laneintersections"] > 0
        ) and not self.config.obs_config.disable_lane_invasion_sensor

        # Conditions to check there is obstacle or red light ahead for last 2 timesteps
        obstacle_ahead = (
            self.episode_measurements["obstacle_dist"] != -1
            and self.prev_measurement["obstacle_dist"] != -1
        )
        red_light = (
            self.episode_measurements["red_light_dist"] != -1
            and self.prev_measurement["red_light_dist"] != -1
        )
        ...
        if success:
            ...
            if self.episode_measurements["obs_collision"]:
                ...
                not self.config.obs_config.disable_lane_invasion_sensor
                and self.episode_measurements["out_of_road"]
            ):
                termination_state = "out_of_road"
                termination_state_code = 2
            elif (
                not self.config.obs_config.disable_lane_invasion_sensor
                and self.episode_measurements["lane_change"]
            ):
        ...
        self.episode_measurements["termination_state_code"] = termination_state_code
            ...
            success or collision or runover_light or offlane or static or maxStepsTaken
        )
        return done
                ...
                for key, val in self.episode_termination_counts.items():
                    ...
                    if self.logger is not None:
                        self.logger.log_scalar(
                            "eval/termination_{}".format(key), val, step
                        )
                print("--------------------------------------------------------------")
...
if __name__ == "__main__":
    ...
    config.populate_config(
        observation_config=obs_config, scenario_config=scenarios_config
    )
    env = CarlaEnv(config=config)
    env.reset()
    for i in range(10000):

        obs, reward, done, info = env.step(np.array([0, 0.5]))
